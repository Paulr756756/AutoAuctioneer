...
namespace Repository_BidStamp.Models
{
    ...
    public class UserRepository : IUserRepository {
        ...
        public async Task<bool> checkUserExists(string email) {
            ...
            if (_db_context.Users.Any(u => u.Email == email)) {
                return true;
            }
            return false;
        }
        ...
        public async Task deleteUser(User user) {
            ...
            List<Stamp> stamps = await _db_context.Stamps.Where(s => s.UserId == user.UserId)
             .ToListAsync();
            List<Listing> listings = await _db_context.Listings.Where(l => l.UserId == user.UserId)
              .ToListAsync();
        ...
        public async Task<bool> getUserByVToken(string token) {
            User user = await _db_context.Users.FirstOrDefaultAsync(u => u.VerificationToken == token);
            if (user != null) {
